...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        rotation_register = 0x0015
        clockwise = 0x00
        counterclockwise = 0x01

        self.encoderB.write_register(rotation_register, counterclockwise, functioncode=6)
    ...
    def resetEnc(self, instrument):
        ...
        angle_value = 0x0064  # Value for 100°
        instrument.write_register(angle_register, angle_value, functioncode=6) 
        print('Angle successfully set to 0')
        print(instrument.read_register(angle_register, functioncode=3, signed=True))
        ...
        read_revolutions = instrument.read_register(revolutions_register, functioncode=3, signed=True) 
    ...
    def calDistance(self):
        val = []
        #EncoderA
        angleA = self.encoderA.read_register(17, 0, signed=True)
        revA = self.encoderA.read_register(18, 0, signed=True)
        val.append(float(angleA)) 
        val.append(float(revA))
        val.append(self.encToAngle(angleA, revA))
        #EncoderB
        angleB = self.encoderB.read_register(17, 0, signed=True)
        revB = self.encoderB.read_register(18, 0, signed=True)
